...
def verify_login():
    ...
    if request.method == "POST":
        ...
        # Verification credentials: smart/smartrun
        verify_user = "smart"
        verify_pass = "smartrun"
...
def verify_required(f):
    """Decorator to require verification login (separate from admin)"""
    @wraps(f)
    def wrapper(*args, **kwargs):
        # Check for verification session (separate from admin session)
        if "verified" not in session or not session.get("verified"):
            flash("Access denied. Verification login required.", "error")
            return redirect(url_for("verify_login"))
        return f(*args, **kwargs)
    return wrapper
...
@login_required
def export_data():
    """Export all registered user data as a formatted text file with clean table formatting"""
    users = fetch_all_users()
    
    # Define column widths - optimized for readability
    col_widths = {
        'sno': 8,
        'name': 25,
        'email': 30,
    ...
    # Header section
    lines.append("=" * 120)
    lines.append("PARTY ENTRY SYSTEM - REGISTERED USERS DATA EXPORT".center(120))
    lines.append("=" * 120)
    ...
    # Data rows - clean format without pipes
    for idx, user in enumerate(users, 1):
        ...
        # Format row with clean spacing (no pipes, just proper spacing)
        row = (
    ...
    lines.append("")
    lines.append("=" * 120)
    lines.append("End of Report".center(120))
    lines.append("=" * 120)
